...
import { stripProcObj, UNDERWATER_PROCOBJ } from '../procobj-strip';
 ...
 * density), emit each as an HD instance + its LOD (text-internal `lod`), and strip those species from
 * `procobj.dat`. {@link UNDERWATER_PROCOBJ} species are never converted (seabed scatter). Returns the gta.dat IPL
 * line to register (or null when nothing qualifies).
 */
export function convertProcObj(options: ProcObjConvertOptions): null | { datLine: string; objects: number } {
  const { archive, gamePath, heightThreshold, iplName, outPath, procObjMax, species } = options;
