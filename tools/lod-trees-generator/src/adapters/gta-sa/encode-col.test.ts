...
describe('encodeColLibrary', () => {
  describe('negative cases', () => {
    it('emits nothing for no impostors', () => {
  ...
  describe('positive cases', () => {
    it('writes a 120-byte COL3 block (8-byte header + 112-byte body) per model', () => {
      const bytes = encodeColLibrary([impostor([0, 0, 0], [1, 1, 1])], ['lodtest']);
    ...
    it('zeroes the counts/offsets/shadow tail (bytes 64..111 of the body), like a stock empty-collision LOD', () => {
      const body = encodeColLibrary([impostor([0, 0, 0], [1, 1, 1])], ['lodtest']).slice(8);

      expect(body.slice(64, 112).every((b) => b === 0)).toBe(true);
    });

    it('names each model from the `names` argument (the IDE alias), not the impostor name', () => {
      const lib = parseColLibrary(ab(encodeColLibrary([impostor([0, 0, 0], [1, 1, 1])], ['lodalias'])));

      expect(lib[0].name).toBe('lodalias');
    });

    it('sets bounds from the bbox (radius = half the diagonal) with no geometry', () => {
      const lib = parseColLibrary(ab(encodeColLibrary([impostor([-3, -4, 0], [3, 4, 20])], ['lodtest'])));
      ...
      expect(lib[0].spheres).toHaveLength(0);
      expect(lib[0].boxes).toHaveLength(0);
    });

    it('emits one block per impostor', () => {
      const models = [impostor([0, 0, 0], [1, 1, 1]), impostor([0, 0, 0], [2, 2, 2])];
      const lib = parseColLibrary(ab(encodeColLibrary(models, ['a', 'b'])));
