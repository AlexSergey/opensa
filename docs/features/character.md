...
- **Skinned model**: Skin plugin → `SkinnedMesh` + `Skeleton`; bones from the frame hierarchy
  ...
  `BMYPOL1`), loaded via `adapter.loadCharacterByModel`.
...
- **Physics** (plans 008/013): bitECS entity + Rapier capsule/box controller, gravity, map
  collision, jump, slope handling; respawn/teleport debug actions. The per-variant spawn
  (`PLAYER_SPAWN[GAME_TYPE]`, `src/ui/locations.ts`) is the single source for where the player starts — it
  seeds both the player capsule and the initial collision zone (`loadGame` centres on it), so there is ground
  under the drop on every variant (Ganton on `original`).
