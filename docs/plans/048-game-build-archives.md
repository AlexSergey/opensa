...
> **Gotcha:** fflate's *streaming* `Zip` produced a corrupt archive on this data (unzip → "invalid distance");
...
1. **Placed models, not all IDE defs.** Collect the model ids actually instanced by the IPLs the variant loads
   ...
   resolve each id → `(modelName, txdName)` via the IDEs (`parseIde` id map). Filtering by *placed* instances
...
> ~15 txd interior-style props that appear in the *exterior* IPLs; the bulk of the interiors is never included.
...
- Use **`zipSync`** (in-memory): fflate's *streaming* `Zip` corrupted the archive on this data (unzip →
