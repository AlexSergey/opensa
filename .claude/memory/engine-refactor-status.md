...
- `ui/` — **the only React surface.** `canvas-host.tsx` (module-scope bootstrap promise = StrictMode-safe; mounts `<canvas>`, forwards resize/click→`game.pick`, shows preloader), `debug/debug-overlay.tsx` (Ctrl+D; drives engine via `Game` methods + `select` event; see [[debug-panel-temporary]]), `locations.ts` (Ganton coords). `main.tsx` renders `<CanvasHost>`.
