...
- **`/object-viewer.html`** (`object-viewer.ts`) — map models; prelit/MODULATE2X/lit toggles + a
...
- **`/vehicle-viewer.html`** (`vehicle-viewer.ts`) — reuses `buildVehicle` + `buildCollisionWireframe`.
  ...
  half-extents.
- **`/character-viewer.html`** (`character-viewer.ts`) — reuses `buildSkinnedClump` + `orientCharacter`
